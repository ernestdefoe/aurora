...
/**
 * Gradient swatch button + popover that lets the visitor pick one of the
 * preset palettes. Rendered via extend(HeaderSecondary.prototype, 'items', …)
 * — being a real Mithril component means it survives the SPA reconciler
 * tearing the header subtree down on navigation / notification updates. The
 * previous imperative installPalettePicker() was attached once at
 * DOMContentLoaded and disappeared on the first route change (audit F3).
 *
 * All user-facing strings go through app.translator.trans() against the
 * ernestdefoe-aurora.forum.palette.* keys (audit F6/F7).
 */
export default class PaletteButton extends Component {
        ...
        <div className="AuroraPalettePopover-grid">
          {Object.entries(PALETTES).map(([name, p]) => (
            <button
              ...
              data-palette={name}
              aria-label={app.translator.trans('ernestdefoe-aurora.forum.palette.apply', {
  ...
  /**
   * Localized palette name, falling back to the built-in English label on
   * PALETTES.<name>.label when the locale has no entry — keeps custom
